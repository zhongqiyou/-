<template>
    <div>
          ...
          <div class="land_user">
              <p><img src="../../public/static/icon/user.jpg" width="100%;" alt=""></p> 
          </div>
          ...
          <div v-else>
           ...
           <van-divider :style="{ color: '#cccccc', borderColor: '#999999', padding: '0 16px' , fontSize:'0.16rem', marginTop:'2.05rem'}">
                 其他登陆方式
            </van-divider>
        ...
        <p class="land_treaty">登录即代表您已同意<span style="color:#4a90e2;">《万物商城隐私政策以及条约》</span></p>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        if(this.pass_phone == this.random_number && this.user_phone == "13824823170"){
                Dialog.alert({
                    title: '登陆成功',
                    message:" 这只是一个测试 ， 不跳转"
                    }).then(() => {
        ...
        onselect(){
             this.axios.get('/wwsc/land/select.php' , {
                 params:{
                    ...
                    .then((res)=>{
                       if(res.data == "登陆成功"){
                               ...
                               setTimeout(()=>{
                                    this.onUserName()
                               this.$router.push({path:'/my',query: {username:this.username , exit:true}});
                              } , 2000)   
        ...
        onUserName(){
             this.axios.get('/wwsc/user/insert.php' , {
                 params:{
                     username:this.username ,
                 }
                })    
