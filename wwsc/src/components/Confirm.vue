<template>
  <div style="background:#eeeeee;padding-bottom:0.1rem;">
    <ul class="land_hander">
      <li v-on:click="onUpper">
        <p class="land_hander_back"></p>
      </li>
    ...
    <ul class="confirm_position" v-on:click="onAddrChange">
      <li style=" width: 20%;text-align:center;">
        <img src="../../public/static/icon/position.png" width="80%" alt />
      </li>
    ...
    <div class="confirm_check">
      <p style="margin:5px 0 10px 0;">
        应付金额
        <span style="color:#de3d96;font-weight:bold;">￥{{$route.query.total-price}}</span>
      </p>
...
export default {
  ...
  methods: {
    ...
    onSelect(user, price) {
      this.axios
        .get("/wwsc/cart/select_commodity.php", {
          params: {
            user: user,
            price: price
          }
    ...
    if (typeof this.$route.query.cart_commodity == "string") {
      ...
      for (let i = 0; i < this.$route.query.cart_commodity.length; i++) {
        this.onSelect(
          ...
          this.$route.query.cart_commodity[i]
        );
      }
    }
    this.onBool();
