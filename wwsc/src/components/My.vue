...
<div style="background:#eeeeee;height:6.2rem;">
         <div class="my_hander">
          <van-nav-bar title="我的易购">
            <div slot="right" style="text-align:right;" v-on:click="onPersonal"><img src="../../public/static/icon/set.png" width="10%" style="vertical-align: middle;" alt=""></div>
          </van-nav-bar>
             
             <ul class="my_hander_user">
              
                 <li>
                     <img src="../../public/static/icon/user.jpg" v-on:click="onland" style="border-radius: 0.4rem;" alt="">
                      <span style="vertical-align:-0.3rem;margin-left:10px;">{{username}}</span>
                      <span v-if="exit" style="margin-left:10px;vertical-align:-0.3rem;color:#cece11;font-size:0.14rem;text-decoration:underline;" v-on:click="onExit">[退出]</span>
        ...
        <div class="my_hander_member">
            <p><img src="../../public/static/icon/menber.png" width="20%" alt=""><span>[免费]你可领取一张运费卷</span></p>
            <p><span style="vertical-align: -17px;color:#777777;">立刻领取</span></p>
        </div>

        <div class="my_main_nav">
             <p style="margin-bottom:10px;"><strong>我的订单</strong></p>
             <ul>
         ...
         <div class="my_main_nav">
             <p style="margin-bottom:10px;"><strong>必备工具</strong></p>
         ...
         onPersonal(){
           if(this.username == "未登录"){
             Toast.fail("用户未登录");
              ...
              this.$router.push({path:"/personal" , query:{user:this.username}});
          }
         }
     },
...
.van-nav-bar{
     background: -webkit-gradient(linear, left top, right top, from(#eb3c3c), to(#ff7459)) !important; 
}
.van-nav-bar__title{
   font-weight: bold;
   color: #ffffff;
