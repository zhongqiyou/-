  ...
  <div style="background:#eeeeee; overflow: auto;"  v-on:scroll="handleScroll()">
    ...
    <div class="detailed_title" v-for="(value , index) in list" :key="index">
         <p class="detailed_title_p1">
          <span style="background:#de3d96;color:#fff;border-radius:5px;font-size:0.15rem;padding:5px;">疯抢价</span>
          <span style="color:#eb5211;font-size:0.24rem;font-weight:bold;">￥{{value.price}}</span>
          <span style="font-size:0.14rem;color:#cccccc; text-decoration: line-through;">{{value.primary}}</span>
    ...
    <div class="detailed_color">
        <p><strong>款式</strong></p>
        <ul>
            <li v-for="(value , index) in color_arr" :key="index" ref="oncolor" v-on:click="onColorChange(index , value.text)">
                <p><img :src="value.src" width="100%" alt=""></p>
                <p>{{value.text}}</p>
            </li>
        </ul>
    </div>
    ...
    <div class="detailed_size">
          <div class="detailed_size_div">
               <p style="font-size:0.14rem;"><strong>尺码</strong></p>
               <p style="color:#4a90e2;">查看尺码表</p>
          </div>

          <ul>
            <li v-for="(value , index) in size" :key="index" ref="onsize" v-on:click="onSizeChange(value)">{{value}}</li>
          </ul>
    </div>
    ...
    <div class="detailed_addr">
         <p>
            ...
            <img src="../../public/static/icon/addr.png" width="5%;" style=" vertical-align: middle;" alt="">
         </p>
      ...
      <div class="detailed_package"> 
           <ul>
                <li v-for="(value , index) in package_text" :key="index"><img src="../../public/static/icon/package.png" alt="">{{value}}</li>
           </ul>
      </div>
      ...
      <div class="detailed_user">
           ...
           <ul>
                <li>
                  <img src="../../public/static/detailed/user.png" style="vertical-align: -0.07rem;margin-right:5px;" width="10%" alt="">
      ...
      <div class="detailed_detailed">
            ...
            <p style="color:rgb(74, 144, 226);font-size:0.12rem;text-align:right;" v-on:click="onOpen">{{open}}</p>
      </div>

      <div class="detailed_contact">
             <p><strong>售后与咨询</strong></p>
             <p><img src="../../public/static/detailed/contact.png" alt="">购买咨询</p>
      </div>

       <div class="detailed_image" style="font-size:0; margin-bottom:0;">
             <p><strong>图文详情</strong></p>
      ...
      <div style="font-size:0;background:#ffffff;">
            ...
            <div class="detailed_explain">
                  ...
                  <p style="color:#999999;margin:0;">{{onOpenExplainArr}}</p>
                    <p style="color:rgb(74, 144, 226);font-size:0.12rem;text-align:right;margin:0;" v-on:click="onExplainOpen">{{explain_open}}</p>
            </div>
      </div>
...
Vue.use(Dialog).use(Toast).use(ImagePreview);
...
export default {
  data() {
    return {
      ...
      package_text:["万物自营" , "7天无理由退货" , "退货返还万物币" , "7天可换" , "买赠唯品币"] ,
      store_image:[
      ...
      detailed_text:[
           {title:"产地" , text:"中国"} , 
           {title:"领型" , text:"翻领"} , 
           {title:"风格" , text:"日韩 , 休闲 , 时尚 , 潮流 , 百搭 , 学院风 , 牛仔系 , 青春活力 , 基础大众"} , 
           {title:"流行元素" , text:"多口袋 , 纽扣 , 直下摆"} , 
           {title:"适用场合" , text:"约会 , 休闲 , 日常 , 通勤 , 校园 , 旅游 , 逛街"} , 
           {title:"版型" , text:"直筒"} , 
           {title:"厚薄" , text:"加绒加厚"} , 
           {title:"弹性" , text:"无弹"} , 
           {title:"适用季节" , text:"冬"} , 
           ...
           {title:"适用性别" , text:"男"} , 
           {title:"材质" , text:"面料：100%棉大身里：100%聚酯纤维袖里：100%聚酯纤维填充物：100%聚酯纤维"} , 
           {title:"洗涤说明" , text:"最高洗涤温度30°C ，常规程序，不可漂白，在阴凉处悬挂晾干 ，熨斗底板最高温度110°C ，不可干洗"} , 
  ...
  methods: {
    onAddCart(){
  if(this.user){
 if(this.size_detailed && this.color_detailed && this.user && this.title_detailed && this.price_detailed && this.src_detailed){
      this.onCart();
      this.onLength();
     }else{
      Toast.fail("请选择码数和款式");
   ...
   this.$router.push({path:"/land"});
    ...
    onCart(){
        this.axios.get('/wwsc/cart/insert.php' , {
              params:{
  ...
  this.$router.go(-1);
   ...
   onScrollClass(){
    let scroll = window.event.path[1].scrollY;
      ...
      onSelect(){
         this.axios.get('/wwsc/detailed/select.php' , {
              params:{
            ...
            .then((res)=>{
             this.list = res.data;
             this.images = [res.data[0].src  ,  "../static/male/male_two.webp" ,  "../static/male/male_three.webp" , "../static/male/male_four.webp" , "../static/male/male_five.webp"];
             this.color_arr=[
   ...
   onSelectUser(){
    ...
    onLength(){
       this.axios.get('/wwsc/cart/length.php' , {
             params:{
                user: this.user
             }
       })
...
.detailed_user p , .detailed_image p{
font-size:0.14rem;
margin:0;
}
.detailed_user p+p{
...
.detailed_package ,.detailed_store ,.detailed_user , .detailed_detailed ,.detailed_contact ,.detailed_image{
   padding:10px;
    margin: 10px 0;
    background: #ffffff;
}
