<template>
         <div style="font-size:0.12rem;background:#eeeeee;height:5rem;">
            ...
            <div class="cart_one" v-if="cart_commodity_swi"> 
                ...
                <li><img src="../../public/static/icon/shop.png" width="15%" style="vertical-align: -0.05rem;margin:0 5px;" alt="">森马男装专卖店<span style="color:#777777;">＞</span></li>
                <li style="color:#eb3c3c;" v-on:click.stop="onVolume">领劵</li>
                   </ul>
                </van-checkbox>
                <van-checkbox-group v-model="result" ref="checkboxGroup">
                    <div v-for="(value , index) in list" :key="index" v-on:click="onIndex(value.id , value.check)">
              ...
              <div style="text-align:center;color:#777777;padding:50px;" v-if="cart_null_swi">
                ...
                <p style="margin-bottom:80px;"><span style="background:#ffffff;padding:5px;border-radius:5px;">去逛逛</span></p>
                <van-divider :style="{ color: '#cccccc', borderColor: '#999999', padding: '0 16px' }">
                没有更多啦
                </van-divider>
              </div>
         <van-submit-bar 
        ...
        >
          <span slot="tip">
                <van-notice-bar
                    text="温馨提示：在双十一活动期间购买的商品 , 在72小时后配货派送!!!"
                    left-icon="volume-o"
                    />
            </span>
        </van-submit-bar>
...
export default {
    data() {
     ...
     methods: { 
       ...
       onUpdateCheck(id , check){
             this.axios.get('/wwsc/cart/update_check.php' , {
            params:{
                user:this.user ,
                id:id ,
                check:check
        ...
        this.axios.get('/wwsc/cart/update.php' , {
            params:{
                user:this.user ,
                id:id ,
   ...
   if(this.select == "提交订单"){
    if(this.result.length){
         this.$router.push({name:'confirm',query: {cart_commodity:this.result , user:this.user , total:this.onTotal}})
        }else{
             Toast.fail("没选择商品");
        }
   }
    ...
    if(this.select == "删除"){   
     ...
     onVolume(){
       ...
       onSelectUser(){
         ...
         onSelect(){
             this.axios.get('/wwsc/cart/select.php' , {
             params:{
                user: this.user
             }
             })
         ...
         onDelete(price){
              this.axios.get('/wwsc/cart/delete.php' , {
             params:{
                user:this.user ,
                price: price
                }
         ...
         whoChech(check){
                 this.axios.get('/wwsc/cart/whol_chech.php' , {
              params:{
                check:check
              }
            })    
