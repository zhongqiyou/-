<template>
    ...
    <div class="comm_hander">
        <SearchList :title_input="$route.params.text" ></SearchList>
          <ul>
            <li ref="hander_title" v-on:click="onTitleColorChange('0')" class="van-ellipsis" style="color:rgb(222, 61, 150);">全部</li>
            <li  v-on:click="onTitleColorChange('1')"> 
                <van-dropdown-menu class="demo" >
                     <van-dropdown-item  v-model="value1" title="销量" :options="option1" v-on:change="onSales"  />
                </van-dropdown-menu></li>
            <li  ref="hander_title"  v-on:click="onTitleColorChange('2')" class="van-ellipsis">折扣</li>
            <li  v-on:click="onTitleColorChange('3')"> 
                <van-dropdown-menu class="demo">
                     <van-dropdown-item v-model="value2" title="价格" :options="option2" v-on:change="onPrice" />
                </van-dropdown-menu></li>
            
            <li v-on:click="onIconChange"><img src="../../public/static/icon/class_row.png" ref="icon_img" width="80%" alt=""></li>
          </ul>
          <Top></Top>
    </div>
         
<div style="margin-top:0.94rem">    
      <List :class_col_row="class_col_row" :title_text="$route.query.text" :volume_desc="volume_desc"></List>
         </div>
   </div>
</template>
...
export default {
    ...
    methods: {
        ...
        onTitleColorChange(index){
                this.hander_text_color_change =  document.querySelectorAll(".van-ellipsis");
